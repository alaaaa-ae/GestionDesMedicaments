    ...
    public class GestionCommandesForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            var lblDetails = new Label
            {
                ...
                Font = new Font("Segoe UI", 12F, FontStyle.Bold),
                ForeColor = Color.FromArgb(50, 50, 60),
                Location = new Point(20, 500), // Position initiale qui sera ajustée
                AutoSize = true
            };

            // --- ListView Commandes ---
            this.listViewCommandes.Location = new Point(20, 210);
            this.listViewCommandes.Size = new Size(1200, 280); // Taille initiale
            this.listViewCommandes.View = View.Details;
            this.listViewCommandes.FullRowSelect = true;
            this.listViewCommandes.GridLines = true;
            this.listViewCommandes.BorderStyle = BorderStyle.FixedSingle;
            ...
            {
                Location = new Point(20, 530), // Position initiale qui sera ajustée
            ...
            // --- ListView Détails ---
            this.listViewDetails.Location = new Point(440, 530); // Position initiale qui sera ajustée
            this.listViewDetails.Size = new Size(780, 170); // Taille initiale
            this.listViewDetails.View = View.Details;
            this.listViewDetails.FullRowSelect = true;
            this.listViewDetails.GridLines = true;
            this.listViewDetails.BorderStyle = BorderStyle.FixedSingle;
            ...
            this.btnNouveau.Location = new Point(20, 720); // Position initiale qui sera ajustée
            ...
            this.Resize += (s, e) => AjusterTailles();
        }
        ...
        private void AjusterTailles()
        {
            ...
            int topOffset = 210; // Position Y de la liste des commandes
            int middleOffset = 500; // Position Y du panneau statut/détails
            int bottomOffset = 720; // Position Y des boutons
            int gap = 20; // Espacement entre les éléments

            if (contentWidth > 0)
            {
                // 1. Ajustement du panel de recherche
                ...
                {
                    panelRecherche.Location = new Point(20, 80);
                    panelRecherche.Size = new Size(contentWidth, 80);
                }

                // 2. Ajustement de la liste des commandes
                this.listViewCommandes.Location = new Point(20, topOffset);
                this.listViewCommandes.Size = new Size(contentWidth, 280);
                
                // Ajustement du label de la liste des commandes
                var lblCommandes = this.panelContent.Controls.OfType<Label>().FirstOrDefault(l => l.Text.Contains("Liste des Commandes"));
                if (lblCommandes != null) lblCommandes.Location = new Point(20, topOffset - 30);

                // 3. Calcul de la largeur pour les éléments du bas
                int bottomContentWidth = contentWidth;
                
                // Si l'écran est assez large, on met le panneau statut et la liste des détails côte à côte
                if (contentWidth > 800)
                {
                    int bottomPanelWidth = (contentWidth - gap) / 2;
                    
                    // Panel Statut
                    var panelStatut = this.panelContent.Controls.OfType<Panel>().FirstOrDefault(p => p.Controls.Contains(comboBoxStatut));
                    if (panelStatut != null)
                    {
                        panelStatut.Location = new Point(20, middleOffset);
                        panelStatut.Size = new Size(bottomPanelWidth, 60);
                    }

                    // ListView Détails
                    this.listViewDetails.Location = new Point(20 + bottomPanelWidth + gap, middleOffset);
                    this.listViewDetails.Size = new Size(bottomPanelWidth, 170);
                    
                    // Ajustement des labels
                    var lblDetails = this.panelContent.Controls.OfType<Label>().FirstOrDefault(l => l.Text.Contains("Détails de la Commande"));
                    if (lblDetails != null) lblDetails.Location = new Point(20 + bottomPanelWidth + gap, middleOffset - 30);
                    
                    var lblStatut = this.panelContent.Controls.OfType<Label>().FirstOrDefault(l => l.Text.Contains("Modifier le statut"));
                    if (lblStatut != null) lblStatut.Location = new Point(20, middleOffset - 30);
                }
                else
                {
                    // Mode une seule colonne pour les petits écrans
                    int elementHeight = 60; // Hauteur du panel statut
                    int detailsHeight = 170; // Hauteur de la liste des détails
                    ...
                    {
                        panelStatut.Location = new Point(20, middleOffset);
                        panelStatut.Size = new Size(contentWidth, elementHeight);
                    }

                    // ListView Détails
                    this.listViewDetails.Location = new Point(20, middleOffset + elementHeight + gap);
                    this.listViewDetails.Size = new Size(contentWidth, detailsHeight);
                    
                    // Ajustement des labels
                    var lblDetails = this.panelContent.Controls.OfType<Label>().FirstOrDefault(l => l.Text.Contains("Détails de la Commande"));
                    if (lblDetails != null) lblDetails.Location = new Point(20, middleOffset + elementHeight + gap - 30);
                    
                    var lblStatut = this.panelContent.Controls.OfType<Label>().FirstOrDefault(l => l.Text.Contains("Modifier le statut"));
                    if (lblStatut != null) lblStatut.Location = new Point(20, middleOffset - 30);
                    
                    bottomOffset = middleOffset + elementHeight + gap + detailsHeight + gap;
                }

                // 4. Ajustement des boutons
                this.btnNouveau.Location = new Point(20, bottomOffset);
                this.btnModifier.Location = new Point(150, bottomOffset);
                this.btnSupprimer.Location = new Point(280, bottomOffset);
                this.btnImprimer.Location = new Point(410, bottomOffset);
                this.btnStats.Location = new Point(540, bottomOffset);
            }
        }

        // Les autres méthodes restent identiques à votre code d'origine...
        // (LoadClients, ListViewCommandes_SelectedIndexChanged, etc.)
